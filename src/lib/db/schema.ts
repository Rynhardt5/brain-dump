...
export const brainDumpItems = pgTable('brain_dump_items', {
  id: uuid('id').defaultRandom().primaryKey(),
  brainDumpId: uuid('brain_dump_id').notNull().references(() => brainDumps.id, { onDelete: 'cascade' }),
...
export const brainDumpCollaborators = pgTable('brain_dump_collaborators', {
  brainDumpId: uuid('brain_dump_id').notNull().references(() => brainDumps.id, { onDelete: 'cascade' }),
...
export const itemVotes = pgTable('item_votes', {
  itemId: uuid('item_id').notNull().references(() => brainDumpItems.id, { onDelete: 'cascade' }),
  userId: uuid('user_id').notNull().references(() => users.id, { onDelete: 'cascade' }),
...
export const brainDumpItemsRelations = relations(brainDumpItems, ({ one, many }) => ({
...
export const brainDumpCollaboratorsRelations = relations(brainDumpCollaborators, ({ one }) => ({
  brainDump: one(brainDumps, {
...
export type NewBrainDumpCollaborator = typeof brainDumpCollaborators.$inferInsert;
